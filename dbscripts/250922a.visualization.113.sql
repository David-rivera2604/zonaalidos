INSERT INTO ALIADOS.VISUALIZATIONS (ID,"SEQUENCE","TYPE",CAPTION,DESCRIPTION,ICON,ICONCLASS,VALUEFORMAT,ENTITYTYPE,RECORDSTATUS,COMPANYID,UPDATEUSERCODE,UPDATEDATE,SPECIFICATION,CONNECTIONNAME,STATEMENTTYPE,DIALOG,STATEMENTEXTEND,"KEY",JAVASCRIPTTOINCLUDE,"STATEMENT") VALUES (113,1,1,'Cobros rechazados','Cobros rechazados',NULL,NULL,NULL,NULL,1,1,111,TIMESTAMP'2025-08-31 00:00:00',TO_CLOB('{
...
}'),'tron',1,NULL,NULL,'CobrosRechazados','Extend.CobrosRechazados.js',TO_CLOB('SELECT DISTINCT null Sel, OP.POLICYID, OP.BILLNUMBER, OP.AMOUNT, OP.CURRENCY Cod_Mon, OP.STATUSDATE, OP.REASON
  ...
  JOIN A2990700 a700 ON a700.NUM_RECIBO  = OP.BILLNUMBER  AND a700.TIP_SITUACION IN (''RE'',''EP'') AND a700.COD_AGT = {app.P_COD_AGT} --AND a700.TIP_GESTOR =''TA''
