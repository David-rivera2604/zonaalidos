...
MERGE INTO VISUALIZATIONS A USING
 (SELECT
  ...
  NULL as ENTITYTYPE,
  1 as TYPE,
  'Todos los casos que tiene acceso el usuario' as CAPTION,
  NULL as DESCRIPTION,
  1 as STATEMENTTYPE,
  'SELECT DISTINCT PC.ID, PC.CONTACTMAINNAME, PC.TITLE, PC.DESCRIPTION, PC.STATUS, LKS.DESCRIPTION STATUSDESC, PC.LABEL, PC.SUBLABEL,
...
WHERE PC.CompanyId=100 AND PC.FLOWID>0 AND (select count(*) from PROCESSSPECSTEP SSC WHERE SSC.FLOWID=PC.FLOWID) > 0
ORDER BY PC.ID DESC,PIS.STARTDATE DESC' as STATEMENT,
  '{
  "title": "Casos",
  "table": {
    "pagination": true,
    ...
    "columns": [
      ...
      },
      {
        ...
        "halign": "center",
        "align": "left",
        "formatter": "app.Ex_Cases.EtapaFormatter",
        "visible": true,
  ...
  'Extend.Cases.js' as JAVASCRIPTTOINCLUDE,
  1 as RECORDSTATUS,
