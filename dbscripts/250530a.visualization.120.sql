UPDATE VISUALIZATIONS SET "TYPE"=1, CAPTION='Lista de casos por devolución de dinero', DESCRIPTION='Lista de casos por devolución de dinero', ICON=NULL, ICONCLASS=NULL, VALUEFORMAT=NULL, ENTITYTYPE=NULL, RECORDSTATUS=1, COMPANYID=1, UPDATEUSERCODE=1, UPDATEDATE=TIMESTAMP '2025-05-28 19:56:10.000000', SPECIFICATION='{
...
SELECT PI.INSTANCEID, PF.NAME
 ...
 WHERE PI.COMPANYID =100 
   ...
   AND PI.TASKID = 0 AND  NOT PI.FINISHDATE IS NULL )   
SELECT PC.ID "Caso #", PINIT.STARTDATE "Inicio", PINIT.FINISHDATE "Completado",
  ...
  WHERE PC.COMPANYID=100 AND TRUNC(PINIT.STARTDATE) >= {desde:date} AND (CAST({hasta:date} AS DATE) IS NULL OR TRUNC(PINIT.FINISHDATE)<={hasta:date})
