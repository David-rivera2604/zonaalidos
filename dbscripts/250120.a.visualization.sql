UPDATE VISUALIZATIONS SET "TYPE"=1, CAPTION='Detalle por caso', DESCRIPTION='Detalle por caso', "STATEMENT"='SELECT CS.ID, LKSCS.DESCRIPTION STATUSDESC, TRUNC(PI.CREATED) CREATEDATE,TO_CHAR(PI.CREATED, ''HH:MI AM'') CREATETIME, CS.CONTACTMAINNAME,
       ...
       PS.NAME STEPNAME,
       CAST (((select count(*) from PROCESSSPECSTEP SSC WHERE SSC.FLOWID=CS.FLOWID AND SSC.ID<=PI.STEPID)*100)/(select count(*) from PROCESSSPECSTEP SSC WHERE SSC.FLOWID=CS.FLOWID) AS DECIMAL(5,0)) Progress,
       ...
       TRUNC(PI.FINISHDATE) FINISHDATE,TO_CHAR(PI.FINISHDATE, ''HH:MI AM'') FINISHTIME, TRUNC(NVL(TO_DATE(PI.FINISHDATE), SYSDATE)) - TO_DATE(PI.STARTDATE) DAYS, TRUNC(NVL(TO_DATE(PI2.FINISHDATE), SYSDATE)) - TO_DATE(PI2.STARTDATE) TOTALDAYS
  FROM PROCESSCASE CS
  LEFT JOIN PROCESSINSTANCE PI ON PI.COMPANYID=CS.COMPANYID AND PI.INSTANCEID=CS.INSTANCEID AND PI.STEPID>0 AND PI.STEPID<=CS.CURRENTSTEPID AND PI.TASKID=0 AND not PI.STARTDATE IS NULL
 ...
 ORDER BY CS.ID, PI.ActivityId', ICON=NULL, ICONCLASS=NULL, VALUEFORMAT=NULL, ENTITYTYPE=NULL, RECORDSTATUS=1, COMPANYID=1, UPDATEUSERCODE=1, UPDATEDATE=TIMESTAMP '2022-06-02 00:00:00.000000', SPECIFICATION='{
...
UPDATE VISUALIZATIONS SET "TYPE"=1, CAPTION='Detalle por caso (excel)', DESCRIPTION='Detalle por caso', "STATEMENT"='SELECT CS.ID "No. Caso", LKSCS.DESCRIPTION "Estatus del caso", TRUNC(PI.CREATED) "Fecha de solicitud (Creada)",TO_CHAR(PI.CREATED, ''HH:MI AM'') "Hora", CS.CONTACTMAINNAME "Contacto", CS.REFERENCE4 "Categoria por correduria", CS.REFERENCE3 "Corredur?a", CS.REFERENCE1 "Tomador", CS.REFERENCE2 "Numero de poliza",
       ...
       CAST (((select count(*) from PROCESSSPECSTEP SSC WHERE SSC.FLOWID=CS.FLOWID AND SSC.ID<=PI.STEPID)*100)/(select count(*) from PROCESSSPECSTEP SSC WHERE SSC.FLOWID=CS.FLOWID) AS DECIMAL(5,0)) "% Progreso",
       ...
       TRUNC(PI.FINISHDATE) "Fecha de finalizacion",TO_CHAR(PI.FINISHDATE, ''HH:MI AM'') "Hora de finalizacion", TRUNC(NVL(TO_DATE(PI.FINISHDATE), SYSDATE)) - TO_DATE(PI.STARTDATE) "Cantidad de Dias", TRUNC(NVL(TO_DATE(PI2.FINISHDATE), SYSDATE)) - TO_DATE(PI2.STARTDATE) "Total de Dias"
  FROM PROCESSCASE CS
  LEFT JOIN PROCESSINSTANCE PI ON PI.COMPANYID=CS.COMPANYID AND PI.INSTANCEID=CS.INSTANCEID AND PI.STEPID>0 AND PI.STEPID<=CS.CURRENTSTEPID AND PI.TASKID=0 AND not PI.STARTDATE IS NULL
  LEFT JOIN PROCESSSPECFLOW PF ON PF.COMPANYID=PI.COMPANYID AND PF.ID=PI.FLOWID
  LEFT JOIN PROCESSSPECSTEP PS ON PS.COMPANYID=PI.COMPANYID AND PS.FLOWID=PI.FLOWID AND PS.ID=PI.STEPID
  ...
  LEFT JOIN PROCESSINSTANCE PI2 ON PI2.COMPANYID=CS.COMPANYID AND PI2.INSTANCEID=CS.INSTANCEID AND PI2.STEPID=0 AND PI2.TASKID=0
 WHERE CS.COMPANYID={app.companyid} AND ({FlowId} =0 OR CS.FLOWID = {FlowId}) AND ({Status} =0 OR CS.STATUS = {Status}) AND TRUNC(PI.CREATED)>={desde:date} AND TRUNC(PI.CREATED)<={hasta:date}
 ORDER BY CS.ID, PI.ActivityId', ICON=NULL, ICONCLASS=NULL, VALUEFORMAT=NULL, ENTITYTYPE=NULL, RECORDSTATUS=1, COMPANYID=1, UPDATEUSERCODE=1, UPDATEDATE=TIMESTAMP '2022-06-02 00:00:00.000000', SPECIFICATION=NULL, CONNECTIONNAME='Research', STATEMENTTYPE=1, DIALOG=NULL, STATEMENTEXTEND=NULL, "KEY"=NULL, JAVASCRIPTTOINCLUDE=NULL WHERE ID=4002 AND "SEQUENCE"=1;