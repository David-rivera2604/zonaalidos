...
PROCEDURE MAPFREREPSINIESTROS (p_COD_AGT NUMBER, p_desde DATE, p_hasta DATE, p_refcur IN OUT SYS_REFCURSOR);
...
WITH balance AS (SELECT ROW_NUMBER () OVER (PARTITION BY ASIGES ORDER BY ID) BLine, b1.* FROM Aliados.PurdyPanelBalance b1 WHERE b1.tipodedocumento = 2),
 ...
 ajuste AS (SELECT ROW_NUMBER () OVER (PARTITION BY ASIGES ORDER BY ID) BLine, b1.* FROM Aliados.PurdyPanelBalance b1 WHERE b1.tipodedocumento = 4)
  SELECT a900.COD_CAUSA_SINI,
       a900.NUM_POLIZA "No.Poliza", A2020.VAL_CAMPO "Placa", A1399T.NOM_TERCERO || ' ' || A1399T.APE1_TERCERO "TOMADOR", A1399G.NOM_TERCERO || ' ' || A1399G.APE1_TERCERO "ACREDOR", dano.Observaciones "Observaciones",
       A1399A.NOM_TERCERO || ' ' || A1399A.APE1_TERCERO "Nombre Asegurado", a80.OBS ASIGES,
       dano.OT "OT", dano.asesorTaller ASESOR, dano.NUM_EXP "EXPEDIENTE",
...
END /DECODE(dano.TipoCambio, NULL, 1, 0, 1, dano.TipoCambio) AS NUMBER(18,2)) "Total Monto indemnizar $",
     ...
     ((SELECT SUM(Monto-NCREPUESTO) FROM Aliados.PurdyPanelBalance bt1 WHERE bt1.NUM_SINI = a80.NUM_SINI AND bt1.NUM_EXP = a80.NUM_EXP AND bt1.ASIGES = a80.OBS AND bt1.tipodedocumento != 3) +
     (SELECT SUM(Monto-NCREPUESTO) FROM Aliados.PurdyPanelBalance bt2 WHERE bt2.NUM_SINI = a80.NUM_SINI AND bt2.NUM_EXP = a80.NUM_EXP AND bt2.ASIGES = a80.OBS AND bt2.tipodedocumento = 3)) "Pendiente x liquidar final",
     ...
     , DECODE(eve.AUTORIZACIONDEUSOPOLIZA, 1, 'Si', 'No') "Autorizaci?n de uso p?liza", eve.FECHAAUTORIZACIONDEUSOPOLIZA "Fecha de autorizaci?n", eve.NUM_EXP
     , CASE 
        WHEN eve.TIPODEINDEMNIZACION IN (1, 2, 3, 10) THEN 'Pagado'
        WHEN eve.TIPODEINDEMNIZACION IN (4, 5, 6) THEN 'Reparación en reserva'
        WHEN eve.TIPODEINDEMNIZACION = 8 THEN 'Cerrado'
        WHEN eve.TIPODEINDEMNIZACION IN (7, 9) THEN 'En reserva'
        ELSE 'En reserva'
    END AS "Estado Mapfre"     
  FROM a7000900 a900
  ...
  JOIN Aliados.PurdyPanelEvento eve ON eve.NUM_SINI = a80.NUM_SINI AND eve.NUM_EXP = a80.NUM_EXP AND eve.ASIGES = a80.OBS
  ...
  LEFT JOIN Aliados.PurdyPanelDano dano ON dano.NUM_SINI = a80.NUM_SINI AND dano.NUM_EXP = a80.NUM_EXP AND dano.ASIGES = a80.OBS
  LEFT JOIN balance b1 ON b1.NUM_SINI = a80.NUM_SINI AND b1.NUM_EXP = a80.NUM_EXP AND b1.ASIGES = a80.OBS AND b1.BLine=1
  LEFT JOIN balance b2 ON b2.NUM_SINI = a80.NUM_SINI AND b2.NUM_EXP = a80.NUM_EXP AND b2.ASIGES = a80.OBS AND b2.BLine=2
  LEFT JOIN balance b3 ON b3.NUM_SINI = a80.NUM_SINI AND b3.NUM_EXP = a80.NUM_EXP AND b3.ASIGES = a80.OBS AND b3.BLine=3
  LEFT JOIN balance b4 ON b4.NUM_SINI = a80.NUM_SINI AND b4.NUM_EXP = a80.NUM_EXP AND b4.ASIGES = a80.OBS AND b4.BLine=4
  LEFT JOIN balance b5 ON b5.NUM_SINI = a80.NUM_SINI AND b5.NUM_EXP = a80.NUM_EXP AND b5.ASIGES = a80.OBS AND b5.BLine=5
  LEFT JOIN balance b6 ON b6.NUM_SINI = a80.NUM_SINI AND b6.NUM_EXP = a80.NUM_EXP AND b6.ASIGES = a80.OBS AND b6.BLine=6
  LEFT JOIN parcialmo pm ON pm.NUM_SINI = a80.NUM_SINI AND pm.NUM_EXP = a80.NUM_EXP AND pm.ASIGES = a80.OBS AND pm.BLine=1
  LEFT JOIN dedu dedu ON dedu.NUM_SINI = a80.NUM_SINI AND dedu.NUM_EXP = a80.NUM_EXP AND dedu.ASIGES = a80.OBS AND dedu.BLine=1
  LEFT JOIN ajuste ajuste ON ajuste.NUM_SINI = a80.NUM_SINI AND ajuste.NUM_EXP = a80.NUM_EXP AND ajuste.ASIGES = a80.OBS AND ajuste.BLine=1
  WHERE a900.COD_CIA=1
    AND a900.COD_AGT=p_COD_AGT
  ORDER BY a900.FEC_SINI DESC;
END PURDYREPSINIESTROS;

PROCEDURE MAPFREREPSINIESTROS (p_COD_AGT NUMBER, p_desde DATE, p_hasta DATE, p_refcur IN OUT SYS_REFCURSOR) IS
BEGIN
  OPEN p_refcur FOR
 WITH base AS (SELECT PB.NUM_SINI, PB.NUM_EXP, PB.ASIGES, PB.FECHA, DECODE(PB.TIPODEDOCUMENTO, 1,'Factura MO (x pagar)', 2,'Factura RE (x pagar)', 3,'Cobro deducible (x cobrar)', 4,'Ajuste (gasto)') TIPODEDOCUMENTO, PB.MONTO MONTO, PB.NCREPUESTO NCREPUESTO
   FROM ALIADOS.PURDYPANELBALANCE PB)
SELECT DISTINCT PB.NUM_SINI NUM_SINI, a700.NUM_EXP, NULL COD_CTO_RVA, NULL ANIO_MES, a900.COD_CIA,
       ...
       PB.MONTO PAGOS, NULL PAGOS_COA, NULL RESERVA_ACT, NULL RESERVA_ANT, NULL RESERVA_PEND_LIQ,
       NULL TIP_RESERVA, NULL COD_TRAMITADOR, NULL FEC_REAP_EXP, PB.FECHA FEC_ACTU, NULL MCA_JUICIO,
       ...
       NULL COD_COB, NULL NOM_COB, a900.COD_CAUSA_SINI, G200.NOM_CAUSA
     , a700.IMP_LIQ IMPORTE_MVTO_LIQUIDADO, a700.FEC_ULT_LIQ FECHA_ULTIMO_PAGO, (SELECT MAX(a1700.NUM_LIQ) FROM A3001700 a1700 WHERE a1700.NUM_SINI=a900.NUM_SINI AND a1700.NUM_EXP=a700.NUM_EXP) NUMERO_ULTIMA_LIQUIDACION
     , PB.TIPODEDOCUMENTO TIPODEDOCUMENTO, PB.ASIGES
  FROM base PB
  JOIN a7000900 a900 ON a900.NUM_SINI=PB.NUM_SINI AND (NVL(p_COD_AGT,0)=0 OR a900.COD_AGT=p_COD_AGT)
  LEFT JOIN A7001000 a700 on a700.NUM_SINI=a900.NUM_SINI AND a700.NUM_EXP = PB.NUM_EXP
  LEFT JOIN A1001399 A1399T ON A1399T.COD_CIA=a900.COD_CIA AND A1399T.TIP_DOCUM = a900.TIP_DOCUM_TOMADOR AND A1399T.COD_DOCUM = a900.COD_DOCUM_TOMADOR
  LEFT JOIN G7000200 G200 ON G200.COD_CIA = a900.COD_CIA AND G200.TIP_CAUSA = 1 AND G200.COD_CAUSA = a900.COD_CAUSA_SINI
  WHERE PB.FECHA >= p_desde AND PB.FECHA <= p_hasta
  ORDER BY PB.NUM_SINI, a700.NUM_EXP;
