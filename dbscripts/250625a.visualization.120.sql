DELETE FROM VISUALIZATIONS v WHERE id = 120;
...
SELECT PI.INSTANCEID, PF.NAME
 ...
 WHERE PI.COMPANYID =100 
   ...
   AND TRUNC(PI.STARTDATE) >= TRUNC({desde:date})
   AND (CAST({hasta:date} AS DATE) IS NULL OR TRUNC(PI.STARTDATE) <= TRUNC({hasta:date}))
)   
