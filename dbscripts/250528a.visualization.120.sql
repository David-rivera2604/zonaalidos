...
SELECT PI.INSTANCEID, PF.NAME
 ...
 WHERE PI.COMPANYID =100 
   AND PI.STEPID = (SELECT ID FROM aliados.PROCESSSPECSTEP WHERE flowid = PI.FLOWID AND NAME = ''SE TRAMITA DEVOLUCIÓN EN BANCOS'')
...
SELECT PC.ID "Caso #", PINIT.STARTDATE "Inicio", PINIT.FINISHDATE "Completado",
  ...
  WHERE PC.COMPANYID=100 AND (PINIT.STARTDATE) >= {desde:date} AND ({hasta:date} IS NULL OR TRUNC(PINIT.FINISHDATE)<={hasta:date})
  ORDER BY PC.ID');