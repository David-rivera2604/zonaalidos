...
WITH base AS (SELECT PB.NUM_SINI, PB.NUM_EXP, PB.ASIGES, PB.FECHA, DECODE(PB.TIPODEDOCUMENTO, 1,'Factura MO (x pagar)', 2,'Factura RE (x pagar)', 3,'Cobro deducible (x cobrar)', 4,'Ajuste (gasto)') TIPODEDOCUMENTO, PB.MONTO MONTO, PB.NCREPUESTO NCREPUESTO
   ...
   cob AS (SELECT DISTINCT a80.NUM_SINI, h7.COD_COB, A2150.NOM_COB
       FROM ALIADOS.PURDYPANELEVENTO a80
       JOIN a7000900 a900 on a900.NUM_SINI=a80.NUM_SINI
       JOIN H7001200 h7 ON h7.COD_CIA= a900.COD_CIA AND h7.NUM_SINI = a900.NUM_SINI AND h7.TIP_EST_COB='P' AND h7.NUM_EXP=(SELECT MAX(h7b.NUM_EXP) FROM H7001200 h7b WHERE h7b.COD_CIA= a900.COD_CIA AND h7b.NUM_SINI = a80.NUM_SINI AND h7b.TIP_EST_COB='P' AND h7b.COD_COB IN (3004, 3005, 3009))
LEFT JOIN A2000040 A40
   ON A40.COD_CIA = a900.COD_CIA
...
SELECT DISTINCT PB.NUM_SINI NUM_SINI, a700.NUM_EXP, NULL COD_CTO_RVA, TO_CHAR(PB.FECHA, 'YYYYMM') ANIO_MES, a900.COD_CIA,
       ...
       cob.COD_COB COD_COB, cob.NOM_COB NOM_COB, a900.COD_CAUSA_SINI, G200.NOM_CAUSA
  ...
  LEFT JOIN cob on cob.NUM_SINI=a900.NUM_SINI 
