...
WITH balance AS (SELECT ROW_NUMBER () OVER (PARTITION BY ASIGES ORDER BY ID) BLine, b1.* FROM Aliados.PurdyPanelBalance b1 WHERE b1.tipodedocumento = 2),
 ...
 WHERE a.NUM_POLIZA=a900.NUM_POLIZA
   ...
   AND num_recibo IN (select t.num_recibo from a2990700 t where t.cod_cia = a.cod_cia and t.num_poliza = a.num_poliza and t.num_spto = a.num_spto and t.num_apli = a.num_apli and t.num_spto_apli = a.num_spto_apli and t.tip_situacion = 'CT')) "PrimasPagadas",
       ...
       DECODE(eve.enviadoaInvestigacion, 1, 'Si', 'No') "Enviado a Investigación", DECODE(eve.posiblesubrogacion, 1, 'Si', 'No')  "Enviado a subrogación POR PURDY-posible sub", eve.FECHAPOSIBLESUBROGACION "Fecha enviado a Subrogacion",
       ...
       NVL(dano.perdidatmonto,0) +(NVL(dano.prerepuestos,0) - NVL(dano.prerepuestosdesc,0)) + NVL(dano.premano,0) "Monto Total Reparación sin IVA",
       NULL "Total Rep con depresación", NULL "NC de depreciacion rep", NULL "Total de la depreciacion", NULL "Infraseguro", NULL "Mo Otros Cargos", NULL "Total Otros Cargos Cliente",
...
END /DECODE(dano.TipoCambio, NULL, 1, 0, 1, dano.TipoCambio) AS NUMBER(18,2)) "Total Monto indemnizar $",
       dano.fechasolicitado "Fecha dano Oculto", dano.observaciones "Daño Oculto Observación",
     ...
     (SELECT SUM(Monto-NCREPUESTO) FROM Aliados.PurdyPanelBalance bt2 WHERE bt2.ASIGES = a80.OBS AND bt2.tipodedocumento = 3)) "Pendiente x liquidar final",
       ...
       NULL "saldo subrogacion", NULL "SUBROGACION-OBSERVACIONES", NULL "CodigoCXC", NULL "NombredeCXCSubrogación",
     ...
     , DECODE(eve.AUTORIZACIONDEUSOPOLIZA, 1, 'Si', 'No') "Autorización de uso póliza", eve.FECHAAUTORIZACIONDEUSOPOLIZA "Fecha de autorización"
  FROM a7000900 a900
...
BEGIN
  OPEN p_refcur FOR
 WITH base AS (SELECT PB.NUMERODESINIESTRO, PB.FECHA, DECODE(PB.TIPODEDOCUMENTO, 1,'Factura MO (x pagar)', 2,'Factura RE (x pagar)', 3,'Cobro deducible (x cobrar)', 4,'Ajuste (gasto)') TIPODEDOCUMENTO, PB.MONTO MONTO, PB.NCREPUESTO NCREPUESTO
   FROM ALIADOS.PURDYPANELBALANCE PB)
SELECT DISTINCT PB.NUMERODESINIESTRO NUM_SINI, a700.NUM_EXP, NULL COD_CTO_RVA, NULL ANIO_MES, a900.COD_CIA,
     ...
     , PB.TIPODEDOCUMENTO TIPODEDOCUMENTO
  FROM base PB     
  JOIN a7000900 a900 ON a900.NUM_SINI=PB.NUMERODESINIESTRO AND (NVL(p_COD_AGT,0)=0 OR a900.COD_AGT=p_COD_AGT)
  LEFT JOIN A7001000 a700 on a700.NUM_SINI=a900.NUM_SINI AND a700.NUM_EXP =1
  LEFT JOIN A1001399 A1399T ON A1399T.COD_CIA=a900.COD_CIA AND A1399T.TIP_DOCUM = a900.TIP_DOCUM_TOMADOR AND A1399T.COD_DOCUM = a900.COD_DOCUM_TOMADOR
  ...
  ORDER BY PB.NUMERODESINIESTRO, a700.NUM_EXP;
