...
BEGIN
  OPEN p_refcur FOR
 WITH base AS (SELECT PB.NUMERODESINIESTRO, PB.FECHA, DECODE(PB.TIPODEDOCUMENTO, 1,'Factura MO (x pagar)', 2,'Factura RE (x pagar)', 3,'Cobro deducible (x cobrar)', 4,'Ajuste (gasto)') TIPODEDOCUMENTO, SUM(PB.MONTO) MONTO, SUM(PB.NCREPUESTO) NCREPUESTO
   FROM ALIADOS.PURDYPANELBALANCE PB
 GROUP BY PB.NUMERODESINIESTRO, PB.FECHA, PB.TIPODEDOCUMENTO)
