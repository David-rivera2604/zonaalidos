        ...
        <xsl:if test="$TipoIdentificacionAsegurado = 2">
          <xsl:choose>
            <xsl:when  test="not(user:cedulaResidencia($IdAsegurado))">
              <xsl:value-of select="concat('El número de identificación del Asegurado con cédula: ',$IdAsegurado,' no corresponde con el tipo de identificación: 2c, Póliza número: ',$NumeroPoliza,'|&#10;')"/>
            </xsl:when >
            <xsl:when test="not(starts-with(normalize-space($IdAsegurado),'1'))">
              <xsl:value-of select="concat('El número de identificación del Asegurado  con cédula: ',$IdAsegurado,' no corresponde con el tipo de identificación: 2d, Póliza número: ',$NumeroPoliza,'|&#10;')"/>
            </xsl:when>
          </xsl:choose>
        </xsl:if>
              ...
              <xsl:if test="$TipoIdentificacion = 2">
                <xsl:choose>
                  <xsl:when  test="not(user:cedulaResidencia($IdBeneficiario))">
                    <xsl:value-of select="concat('El número de identificación del beneficiario con cédula: ',$IdBeneficiario,' no corresponde con el tipo de identificación: 2e, Póliza número: ',$NumeroPoliza,'|&#10;')"/>
                  </xsl:when >

                  <xsl:when test="not(starts-with(normalize-space($IdBeneficiario),'1'))">
                    <xsl:value-of select="concat('El número de identificación del beneficiario con cédula: ',$IdBeneficiario,'  no corresponde con el tipo de identificación: 2f, Póliza número: ',$NumeroPoliza,'|&#10;')"/>
                  </xsl:when>
