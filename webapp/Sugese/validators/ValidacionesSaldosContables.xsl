<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt" xmlns:myObj="urn:bccrfunciones">
  ...
  <xsl:template name="SumarAtributosYElementos">    
    ...
    <xsl:variable name="valores">
      <xsl:for-each select="./*">
        <xsl:choose>
          <xsl:when test="count(./@*) > 0"> <!--Si tiene atributos-->
            <xsl:value-of select="concat(./@*[1], '|')"/>
          </xsl:when>
          <xsl:otherwise>
            <xsl:value-of select="concat(., '|')"/> <!--Si tiene elementos-->
          </xsl:otherwise>
        </xsl:choose>
      </xsl:for-each>
    </xsl:variable>
  ...
  <!--
  Valida una cuenta madre
  1. Si tiene un total comprueba que tenga cuentas hijas
  2. Si tiene un total comprueba que la suma de las cuentas hijas sumen el total  
  -->
