...
app.EmisionAccidentesPersonales = (function () {
    ...
    function Setup() {
        ...
        if (_id != '') {
            ...
            app.core.Get(app.setting.apipath + 'v1/Issue/AccidentesPersonales/' + _id + '?mode=' + workMode)
                .done(function (data) {
                    Init_Lookups(data);
                });
        }
    ...
    function Quote() {
        app.core.Post(app.setting.apipath + 'v1/Issue/AccidentesPersonales',
    ...
    function ReadOnly() {
        ...
        $('#COD_OCUPACION').replaceWith('<div>' + $('#COD_OCUPACION option:selected').text() + '</div>');
        $('#COD_DEDUCIBLE').replaceWith('<div>' + $('#COD_DEDUCIBLE option:selected').text() + '</div>');
    };
    ...
    function MapObjectToInput(data) {
        ...
        app.ui.SetDropDownNumericValue('#COD_MODALIDAD', data.cod_mon, true);
        $('#COD_OCUPACION').val(data.COD_OCUPACION);
        app.ui.SetDropDownNumericValue('#COD_OCUPACION', data.COD_OCUPACION, true);
        $('#COD_DEDUCIBLE').val(data.COD_DEDUCIBLE);
        app.ui.SetDropDownNumericValue('#COD_DEDUCIBLE', data.COD_DEDUCIBLE, true);
...
window.tercerosTbl_Events = {
    'click .delete': function (e, value, row, index) {
        toastr.warning("Si está seguro de querer eliminar el tercero '" + row.nombre + "' haga clic aquí", null, { timeOut: 5000, closeButton: true, progressBar: true, onclick: function () { app.EmisionAccidentesPersonales.tercerosDeleteRow(row); } });
        e.stopPropagation();
    },
    'click .edit': function (e, value, row, index) {
        app.EmisionAccidentesPersonales.tercerosEditRow(row);
        e.stopPropagation();
    }
};
window.documentosrequeridosTbl_Events = {
    'click .delete': function (e, value, row, index) {
        toastr.warning("Si está seguro de querer limpiar el documento requerido '" + row.DNombre + "' haga clic aquí", null, { timeOut: 5000, closeButton: true, progressBar: true, onclick: function () { app.EmisionAccidentesPersonales.documentosrequeridosDeleteRow(row); } });
        e.stopPropagation();
    },
    'click .edit': function (e, value, row, index) {
        app.EmisionAccidentesPersonales.documentosrequeridosEditRow(row);
        e.stopPropagation();
    }
};