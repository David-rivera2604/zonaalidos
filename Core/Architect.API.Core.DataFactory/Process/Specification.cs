...
namespace Architect.API.Core.DataAccess.General.Process
{
    public static class Specification
    {
        ...
        public static Contracts.General.ProcessSpecFlow Retrieve(int flowId, int companyId)
        {
            ...
            Contracts.General.ProcessSpecFlow result = Core.DataAccess.General.ProcessSpecFlow.Retrieve(flowId, companyId, currentConnection);

            if (result.IsNotEmpty())
            {
                result.ProcessSpecSteps = Core.DataAccess.General.ProcessSpecStep.RetrieveByFlowId(companyId, flowId, currentConnection);
                if (result.ProcessSpecSteps.IsNotEmpty())
                {
                    ...
                    {
                        item.ProcessSpecStepRoles = Core.DataAccess.General.ProcessSpecStepRole.RetrieveByStepId(item.Id, currentConnection);
                        item.ProcessSpecTasks = Core.DataAccess.General.ProcessSpecTask.RetrieveByStepId(companyId, item.Id, currentConnection);
