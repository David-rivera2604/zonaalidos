...
namespace Architect.API.Core.Controllers
{
    ...
    [ApiVersion("1.0")]
    [Authorize]
    [RoutePrefix("api/v{version:apiVersion}/LookupMaster")]
    [ApiExplorerSettings(IgnoreApi = true)]
    public class LookupMasterController : ApiController
    {
        /// <summary>
        /// Crea un registro en la tabla LookupMaster.
        /// </summary>
        /// <param name="item">Instancia del objecto LookupMaster.</param>
        /// <returns>Información del registro creado en la tabla LookupMaster.</returns>
        [HttpPost]
        [Route("")]
        [Authorize]
        public async Task<IHttpActionResult> Post([FromBody] Architect.API.Core.Contracts.General.LookupMaster item)
        {
            ...
            await Task.Run(() =>
            {
                ...
                {
                    result = Created(string.Format("{0}/{1}", Request.RequestUri.AbsoluteUri.Substring(0, Request.RequestUri.AbsoluteUri.LastIndexOf("/")), created.LookupMaster.LookupId), new { Id = created.LookupMaster.LookupId, UpdateDate = created.LookupMaster.UpdateDate });
                }
                else
                {
        ...
        /// <returns>Lista de objetos LookupMaster.</returns>
        [HttpGet]
        [Route("")]
        [Authorize]
        public async Task<IHttpActionResult> Get([FromUri] string filter = "", int beginIndex = 1, int endIndex = int.MaxValue)
        {
            ...
            await Task.Run(() =>
            {
                result = Architect.API.Core.Business.General.LookupMaster.Retrieve(tokenInfo.CompanyId, filter, beginIndex, endIndex);
            }).ConfigureAwait(false);
        ...
        public async Task<IHttpActionResult> Count([FromUri] string filter = "")
        {
            ...
            await Task.Run(() =>
            {
                result = Architect.API.Core.Business.General.LookupMaster.Count(tokenInfo.CompanyId, filter);
            }).ConfigureAwait(false);
            return Ok(result);
        }

        /// <summary>
        /// Recupera un registro en la tabla LookupMaster por medio de su identificador único.
        /// </summary>
        /// <param name="id">Identificador único.</param>
        /// <returns>Instancia del objecto LookupMaster.</returns>
        [HttpGet]
        [Route("{id:int}")]
        [Authorize]
        public async Task<IHttpActionResult> GetById([FromUri] int id)
        {
            ...
            if (id.IsEmpty())
            {
                result = BadRequest("Debe indicar el identificador del lookup master");
            }
            else
            {
                await Task.Run(() =>
                {
                    data = Architect.API.Core.Business.General.LookupMaster.RetrieveById(tokenInfo.CompanyId, id);
                    if (data.IsEmpty())
                    {
                        result = NotFound();
                    }
        ...
        /// <summary>
        /// Actualiza un registro en la tabla LookupMaster por medio de su identificador único.
        /// </summary>
        /// <param name="id">Identificador único.</param>
        /// <param name="item">Instancia del objecto LookupMaster.</param>
        /// <returns>Información del registro actualizado en la tabla LookupMaster.</returns>
        [HttpPut]
        [Route("{id:int}")]
        [Authorize]
        public async Task<IHttpActionResult> Put([FromUri] int id, [FromBody] Architect.API.Core.Contracts.General.LookupMaster item)
        {
            ...
            {
                item.LookupId = id;
                Architect.API.Core.Contracts.General.LookupMasterResult updated = Architect.API.Core.Business.General.LookupMaster.Update(tokenInfo.CompanyId, tokenInfo.UserId, id, item);
                if (updated.Errors.Count == 0)
                {
                    result = Ok(new
                    {
                        Id = updated.LookupMaster.LookupId,
                        UpdateDate = updated.LookupMaster.UpdateDate
                    });
                }
                else
                {
        ...
        public async Task<IHttpActionResult> Delete([FromUri] int id)
        {
            ...
            await Task.Run(() =>
            {
                ...
                {
                    result = Ok(deleted.LookupMaster);
                }
                else
                {
