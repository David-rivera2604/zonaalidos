...
namespace Architect.API.Core.Controllers
{
    ...
    public class ProcessCaseController : ApiController
    {
        ...
        public async Task<IHttpActionResult> Count([FromUri] string filter = "")
        {
            ...
            await Task.Run(() =>
            {
                result = Architect.API.Core.Business.General.ProcessCase.Count(tokenInfo.CompanyId, filter);
            }).ConfigureAwait(false);
            return Ok(result);
        }
        ...
        public async Task<IHttpActionResult> GetById([FromUri] int id)
        {
            ...
            Architect.API.Core.Contracts.General.ProcessCase data = null;
            ...
            else
            {
                await Task.Run(() =>
                {
                    data = Architect.API.Core.Business.General.ProcessCase.RetrieveById(tokenInfo.CompanyId, id);
                    if (data.IsEmpty())
                    {
                        result = NotFound();
                    }
        ...
        [Route("{id:int}")]
        [Authorize]
        public async Task<IHttpActionResult> Put([FromUri] int id, [FromBody] Architect.API.Core.Contracts.General.ProcessCase item)
        {
            ...
            {
                item.Id = id;
                Architect.API.Core.Contracts.General.ProcessCaseResult updated = Architect.API.Core.Business.General.ProcessCase.Update(tokenInfo.CompanyId, tokenInfo.UserId, id, item);
