...
#Region "WarningLog"
        ...
        Public Shared Sub WarningLog(source As String, entry As String, prefix As String, customData As Object, Async As Boolean)
            Dim logprefix As String = ConfigurationManager.AppSettings("Logs.Prefix")
            ...
            If Not String.IsNullOrEmpty(ConfigurationManager.AppSettings("FrontOffice.Debug.Mode")) Then
                DebugMode = ConfigurationManager.AppSettings("FrontOffice.Debug.Mode")
            Else
                DebugMode = "File"
            End If
        ...
        Private Shared Function WarningLogInternal(parameterInternal As Dictionary(Of String, Object)) As Boolean
            ...
            Dim _DebugMode As String = parameterInternal.Item("DebugMode")
...
#Region "TraceLog"
        ...
        Public Shared Sub TraceLog(source As String, entry As String, prefix As String, customData As Object, ByVal Async As Boolean)
            ...
            If Not String.IsNullOrEmpty(ConfigurationManager.AppSettings("FrontOffice.Debug")) AndAlso
               ConfigurationManager.AppSettings("FrontOffice.Debug") = "True" Then
                ...
                If Not String.IsNullOrEmpty(ConfigurationManager.AppSettings("FrontOffice.Trace.Mode")) AndAlso
                   ConfigurationManager.AppSettings("FrontOffice.Trace.Mode") = "Single" Then
        ...
        Private Shared Function GetPath() As String
            Dim rootPath As String = ConfigurationManager.AppSettings("Path.Logs")
            ...
            Else
                If rootPath.ToLower.Contains("special") Then
                    ...
                    Else
                        Dim temporal = String.Format("{0}{1}", Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments), rootPath.Replace("special", "").Replace("@", "\"))
                        rootPath = temporal
                    End If
                End If
            End If
...
#Region "ErrorLog"
        ...
        Public Shared Sub ErrorLog(source As String, entry As String, currentException As Exception, prefix As String, Async As Boolean, Code As String)
            ...
            If String.IsNullOrEmpty(prefix) Then
                prefix = ConfigurationManager.AppSettings("Logs.Prefix")
            End If
            ...
            If HttpContext.Current IsNot Nothing Then
                If HttpContext.Current.Request IsNot Nothing Then
                    If HttpContext.Current.Request.ServerVariables IsNot Nothing Then
                        Dim value As String
                        If ConfigurationManager.AppSettings("FrontOffice.Debug.Detail").IsNotEmpty _
                                AndAlso ConfigurationManager.AppSettings("FrontOffice.Debug.Detail").ToLower.Equals("true") Then
...
#Region "Tools"
        ...
        ''' <remarks></remarks>
        Private Shared Sub TraceInnerExceptionMessage2(ByVal ex As Exception, ByVal level As Integer, sw As StringBuilder)
            ...
            With sw
                .Append(String.Format(" {1}({0}) {2}", TypeName(ex), " ".PadLeft(level), ex.Message))
            End With

        End Sub

        Private Shared Sub TraceInnerExceptionData(ByVal ex As Exception, ByVal level As Integer, sw As StringBuilder)
            ...
            With sw
                If ex.Data.Count > 0 Then
                    .AppendLine(String.Format(" {1}({0}) Details:", TypeName(ex), " ".PadLeft(level)))
                    For Each de As DictionaryEntry In ex.Data
                        .AppendLine(String.Format("   Key: {0,-20}  Value: {1}", de.Key, de.Value))
                    Next
                End If
        ...
        ''' <returns></returns>
        ''' <remarks></remarks>
        Private Shared Function TraceInnerExceptionMessage(ByVal ex As Exception, ByVal level As Integer) As String
            Dim result As String = String.Empty
        ...
        Private Shared Function IsExist(stat As StackFrame(), fount As String) As Boolean
            Dim result As Boolean
            For Each Item In stat
                If Item.ToString.ToLower.Contains(fount.ToLower) Then
                    result = True
                    Exit For
                End If
            Next
