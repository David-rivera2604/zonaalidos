...
#Region "Execute Scalar Methods"
    ...
    Public Function QueryExecuteScalarToInteger() As Integer
        ...
        If Not ForceLocalMode Then
            If ConfigurationManager.AppSettings("DataManager.Mode").IsNotEmpty AndAlso
                ConfigurationManager.AppSettings("DataManager.Mode").ToLower = "remote" Then
                local = False
            End If
        Else
            local = True
...
#Region "Tools"
    ...
    Public Shared Function GetDataBaseProvider(connectionStringName As String) As String
        ...
        'If ConfigurationManager.AppSettings("DataManager.URL").IsEmpty Then
        '    dataService = New DataManager.DataManagerClient()
        'Else
        '    dataService = InstanceDataManagerClient(ConfigurationManager.AppSettings("DataManager.URL"))
        'End If
    ...
    Public Function CompanyIdSelect() As Integer
        Dim Result As Integer = ConfigurationManager.AppSettings("BackOffice.CompanyDefault")
    ...
    Public Shared Function ObjectExist(owner As String, type As String, name As String, connectionStringName As String) As Boolean
        ...
        If local Then
            ...
            End With
            'Else
            '    Using dataService As New DataManager.DataManagerClient("BasicHttpBinding_IDataManager",
            '                                                                              String.Format(CultureInfo.InvariantCulture, "{0}/DataManager.svc",
            '                                                                                            ConfigurationManager.AppSettings("DataManager.URL")))
...
#Region "Cache"
    ...
    Public Shared Function GetMd5Hash(command As Services.Contracts.DataCommand) As String
        Using md5Hash As MD5 = MD5.Create()
            Dim data As Byte() = md5Hash.ComputeHash(Encoding.UTF8.GetBytes(command.ConnectionStringName + Common.Helpers.Serialize.Serialize(Of Services.Contracts.DataCommand)(command)))
...
#Region "Package Commands"
    ...
    Public Function AddCommand(statement As String, tableName As String, operation As String) As Services.Contracts.DataCommand
        Dim result As New Services.Contracts.DataCommand With {.ConnectionStringName = _command.ConnectionStringName,
                                            .Statement = statement,
                                            .TableName = tableName,
                                            .Operation = operation}
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Function PackageExecuteScalar() As String
        Dim result As String = String.Empty
        Dim local As Boolean = True
...
#Region "Internal use"
    ...
    '        If ConfigurationManager.AppSettings("Certificate.ForcedSelfSigned").IsNotEmpty AndAlso
    '           ConfigurationManager.AppSettings("Certificate.ForcedSelfSigned").ToLower.Equals("true") Then
    ...
    Private Sub SetConfiguration(command As Services.Contracts.DataCommand)
        If MaxNumberOfRecord.IsEmpty Then
            If ConfigurationManager.AppSettings("DataAccessLayer.MaxNumberOfRecords").IsNotEmpty AndAlso
               ConfigurationManager.AppSettings("DataAccessLayer.MaxNumberOfRecords") <> 0 Then
                MaxNumberOfRecord = (ConfigurationManager.AppSettings("DataAccessLayer.MaxNumberOfRecords"))
            End If
            If ConfigurationManager.AppSettings("DataAccessLayer.IgnoreMaxNumberOfRecords").IsNotEmpty AndAlso
               Not ConfigurationManager.AppSettings("DataAccessLayer.IgnoreMaxNumberOfRecords").Equals("false") Then
