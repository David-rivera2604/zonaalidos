...
namespace Architect.Utilities.Helpers
{
    ...
    public class Assembly
    {
        ...
        {
            return (new Assembly()).GetTypeFromName(typeName, null);
        }
        ...
        public  Type GetTypeFromName(string typeName, string assemblyName)
        {
           ...
           System.Reflection.Assembly[] assemblies = AppDomain.CurrentDomain.GetAssemblies();
            ...
            {
               System.Reflection. Assembly a = Assembly.LoadAssembly(assemblyName);
                if (a != null)
                {
        ...
        public Assembly(int traceLevel = 4)
        {
            //----------------------------------------------------------------------
        ...
        public static string GetNameApplication()
        {
            ...
            if (!HostingEnvironment.IsHosted)
            {
                try
                {
                    result = (new Assembly(5)).Title;
                    if (result.IsEmpty())
                    {
                        result = System.Reflection.Assembly.GetCallingAssembly().GetName().Name;
                    }
                }
                catch (Exception ex)
                {
                    Log.ErrorLog("AssemblyHandler", "GetNameAplication", ex);
        ...
        public static string GetVersionApplication()
        {
            ...
            if (!HostingEnvironment.IsHosted)
            {
                try
                {
                    result = (new Assembly(5)).VersionFull;
                    if (result.IsEmpty())
                    {
                        result = System.Reflection.Assembly.GetCallingAssembly().GetName().Version.ToString();
                    }
                }
