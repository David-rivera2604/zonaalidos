...
{
    public static class AccidentesPersonales
    {
        const int COD_RAMO = 194;
        public static Contracts.Cotizacion.AccidentesPersonales Setup(Core.Contracts.Security.Token tokenInfo)
        {
            Contracts.Cotizacion.AccidentesPersonales result = new Contracts.Cotizacion.AccidentesPersonales()
            {
                cod_mon = 1,
                cod_fracc_pago = 1,
        ...
        public static Contracts.Cotizacion.AccidentesPersonales Quote(Contracts.Cotizacion.AccidentesPersonales quoteInfo, Core.Contracts.Security.Token tokenInfo)
        {
            Contracts.Cotizacion.AccidentesPersonales resultInfo = quoteInfo;
            ...
            if (resultInfo.Errors.Count == 0)
            {
                ...
                Architect.API.Tron.Contracts.Presupuesto.DatoFijo result = AccidentesPersonalesConvert.ToTron(quoteInfo, COD_RAMO, tokenInfo.AgentCode, tokenInfo.UserName);
                result = Backoffice.Cotizacion.Generico.Calcular(result);
                resultInfo = LookupComplements((Contracts.Cotizacion.AccidentesPersonales)Util.FromTron_CoberturasResult(quoteInfo, result, 1, true), tokenInfo);
        ...
        public static Contracts.Cotizacion.AccidentesPersonales LookupComplements(Contracts.Cotizacion.AccidentesPersonales quoteInfo, Core.Contracts.Security.Token tokenInfo)
        {
            return quoteInfo;
        }
        /// <summary>
        ...
        /// <returns></returns>
        private static List<Core.Contracts.General.Error> Validate(Contracts.Cotizacion.AccidentesPersonales source, Core.Contracts.Security.Token tokenInfo)
        {
            //const string group = "Estudiantil";
            const string group = "AccidentesPersonales";
        ...
        private static bool Rule_AtLeastOneCoverageSelected(Contracts.Cotizacion.AccidentesPersonales source)
        {
            bool finded = false;
