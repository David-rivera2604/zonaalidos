...
namespace Architect.API.Tron.Business.Emision
{
    internal static class MapfreMasConvertTo
    {
        ...
        internal static Architect.API.Tron.Contracts.Presupuesto.DatoFijo Tron(Contracts.Emision.MapfreMas quoteInfo)
        {
        ...
        internal static Architect.API.Tron.Contracts.Presupuesto.DatoFijo DatosFijos(Contracts.Emision.MapfreMas quoteInfo, int branch)
        {
            ...
            Architect.API.Tron.Contracts.Presupuesto.DatoFijo datosFijos = new Architect.API.Tron.Contracts.Presupuesto.DatoFijo()
            {
                ...
                num_apli = 0,
                num_spto_apli = 0,
                cod_sector = Convert.ToInt32(ConfigurationManager.AppSettings["Mapfre.Tron.MapfreMas.cod_sector"]),
                cod_ramo = branch,
        ...
        internal static List<Architect.API.Tron.Contracts.Presupuesto.DatoVariable> DatosVariable(Contracts.Emision.MapfreMas quoteInfo, Architect.API.Tron.Contracts.Presupuesto.DatoFijo datosFijos)
        {
            ...
            datosVariables.Add(Util.DatoVariable(datosFijos, num_riesgo, "COD_COLOR", quoteInfo.COD_COLOR.ToString(), 2, 5));
            datosVariables.Add(Util.DatoVariable(datosFijos, num_riesgo, "NUM_MATRICULA", quoteInfo.NUM_MATRICULA, 2, 7));
            datosVariables.Add(Util.DatoVariable(datosFijos, num_riesgo, "NUM_MOTOR", quoteInfo.NUM_MOTOR, 2, 8));
            datosVariables.Add(Util.DatoVariable(datosFijos, num_riesgo, "VAL_CAPACIDAD", quoteInfo.VAL_CAPACIDAD.ToString(), 2, 9));
            datosVariables.Add(Util.DatoVariable(datosFijos, num_riesgo, "DES_TIP_CILINDRAJE", quoteInfo.DES_TIP_CILINDRAJE, 2, 10));
            datosVariables.Add(Util.DatoVariable(datosFijos, num_riesgo, "VAL_PESO", quoteInfo.VAL_PESO.ToString(), 2, 11));
            datosVariables.Add(Util.DatoVariable(datosFijos, num_riesgo, "COD_CHASSIS", quoteInfo.COD_CHASSIS, 2, 12));

            return datosVariables;
        }
        internal static List<Architect.API.Tron.Contracts.Presupuesto.Tercero> Terceros(Contracts.Emision.MapfreMas quoteInfo, Architect.API.Tron.Contracts.Presupuesto.DatoFijo datosFijos)
        {
            ...
            foreach (Contracts.Comun.tercero item in quoteInfo.terceros)
            {
                ...
                else
                {
                    ...
                    if (item.elaseguradoeselconductorhabitual == 1)
                    {
