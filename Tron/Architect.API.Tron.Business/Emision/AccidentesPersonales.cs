...
{
    /// <summary>
    /// Emision de Poliza Accidentes personales en TRON
    /// </summary>
    public static class AccidentesPersonales
    {
        public static Contracts.Emision.AccidentesPersonales Setup(string presupuesto, string mode, Core.Contracts.Security.Token tokenInfo)
        {
            Contracts.Emision.AccidentesPersonales result = null;
            string key = string.Format("AccidentesPersonales.{0}", presupuesto);
            if (mode == "resume")
            {
                ...
                Contracts.Cotizacion.AccidentesPersonales resultInfo2 = Cotizacion.AccidentesPersonalesConvert.FromTron_Full(P30Instance);
        ...
        private static List<Contracts.Comun.tercero> Default_Terceros(Contracts.Emision.AccidentesPersonales quoteInfo)
        {
        ...
        public static Contracts.Emision.AccidentesPersonales Issue(Contracts.Emision.AccidentesPersonales quoteInfo, Core.Contracts.Security.Token tokenInfo)
        {
            ...
            {
                Core.Business.General.ChangeSet.Create(3000, Convert.ToInt32(resultQuoteInfo.num_poliza.Substring(4)), tokenInfo.CompanyId, "Emisión Seguro de Accidentes Personales", "Póliza #" + resultQuoteInfo.num_poliza, tokenInfo.UserId, resultQuoteInfo);
