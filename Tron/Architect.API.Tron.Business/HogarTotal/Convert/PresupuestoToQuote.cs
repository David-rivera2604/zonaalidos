...
namespace Architect.API.Tron.Business.HogarTotal.Convert
{
    internal class PresupuestoToQuote
    {
            ...
            foreach (Contracts.Presupuesto.DatoVariable item in tronQuoteInfo.DatosVariables)
            {
                switch (item.cod_campo)
                {
                    ...
                    case "COD_ESTADO": // PROVINCIA
                        quoteInfo.provincia = System.Convert.ToInt32(item.val_campo);
                        quoteInfo.provinciaDesc = item.txt_campo;
                        break;
                    case "COD_PROV": // CANTÓN
                        quoteInfo.canton = System.Convert.ToInt32(item.val_campo);
                        quoteInfo.cantonDesc = item.txt_campo;
                        break;
                    case "COD_LOCALIDAD": // DISTRITO
                        quoteInfo.distrito = System.Convert.ToInt32(item.val_campo);
                        quoteInfo.distritoDesc = item.txt_campo;
                        break;
                    case "COD_TIPO_OCUP_RGO": // OCUPACIÓN DEL RIESGO
                        quoteInfo.ocupaciondelriesgo = System.Convert.ToInt32(item.val_campo);
                        quoteInfo.ocupaciondelriesgoDesc = item.txt_campo;
                        break;
                    case "TIP_SUSCRIPCION": // TIPO DE SUSCRIPCION
                        quoteInfo.tipodesuscripcion = System.Convert.ToInt32(item.val_campo);
                        quoteInfo.tipodesuscripcionDesc = item.txt_campo;
                        break;
                    case "NUM_PISOS_EDIF": // NÚMERO DE PISOS EDIFICACIÓN
                        quoteInfo.numerodepisosedificacion = System.Convert.ToInt32(item.val_campo);
                        break;
                    case "COD_TIPO_ESTRUC": // TIPO DE ESTRUC DE LA EDIFICACI
                        quoteInfo.tipodeestrucdelaedificacion = System.Convert.ToInt32(item.val_campo);
                        break;
                    case "COB_PDR_MESES": // MESES A AMPARAR POR PERD RENTA
                        quoteInfo.mesesaampararporperdrentas = System.Convert.ToInt32(item.val_campo);
                        break;
                    case "IMP_EDIFICIO": // S.A. EDIFICIO
                        quoteInfo.sAEdificio = System.Convert.ToInt32(item.val_campo);
                        break;
                    case "IMP_MOBILIARIO": // S.A. MOBILIARIO
                        quoteInfo.sAMobiliario = System.Convert.ToInt32(item.val_campo);
                        break;
                    case "IMP_OBJ_VALIOSO": // S.A. OBJETOS VALIOSOS
                        quoteInfo.sAObjetosvaliosos = System.Convert.ToInt32(item.val_campo);
                        break;
                    case "IMP_DOMOS_TOTAL": // S.A. DOMO,CRISTAL,MÁRMOL,GRAN.
                        quoteInfo.sADomocristalmarmolgranito = System.Convert.ToInt32(item.val_campo);
                        break;
                    case "IMP_GAS_ALQ": // S.A. GASTOS DE ALQUILER
                        quoteInfo.sAGastosalquiler = System.Convert.ToInt32(item.val_campo);
                        break;
                    case "IMP_PER_REN": // S.A. PÉRDIDA DE RENTAS
                        quoteInfo.sAPerdidaderentas = System.Convert.ToInt32(item.val_campo);
                        break;
                    case "IMP_SA_RC	S.A.": // RESP. CIVIL
                        quoteInfo.sARespcivil = System.Convert.ToInt32(item.val_campo);
                        break;
                    case "PCT_AJUSTE_GEN": // PORCENTAJE AJUSTE COMERCIAL
                        quoteInfo.descuento = System.Convert.ToInt32(item.val_campo);
