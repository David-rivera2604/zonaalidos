    ...
    public class SINPEMovil
    {
        public async static Task<Contracts.SINPEMovil.Response.AplicarPago> Create(Contracts.SINPEMovil.Request.AplicarPago aplicarPagoRequest)
        {
            ...
            if (id > 0 && result.Codigo == 98)
            {
                ...
                {
                    Contracts.Pagos.Recibo recibo = DataAccess.Pagos.Recibos.PrimerReciboAlCobroPorPoliza(policyNumber);
                    if (recibo != null)
                    {
                        ...
                        await Pagos.PaymentApproved(new InformationRequest()
                        {
                            status = "PENDING",
            ...
            if (id > 0)
            {
                ...
                using (DataFactory.Session session = new DataFactory.Session("Research"))
                {
                    try
                    {
                        DataAccess.Pagos.SINPEMovil.UpdateEstado(id, 2, result.Mensaje, session);
                        session.CommitAndClose();
                    }
                    catch (Exception ex)
                    {
