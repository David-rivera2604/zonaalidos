...
$@"SELECT C.NUM_RECIBO, C.FEC_EFEC_RECIBO, SUM(C.IMP_RECIBO) IMP_RECIBO, A400.COD_MON_ISO NOM_MON, A.NUM_POLIZA, A.COD_AGT, a1800.nom_ramo, a200.NOM_SECTOR, COALESCE(a60.TIP_DOCUM, A.TIP_DOCUM) TIP_DOCUM, COALESCE(a60.COD_DOCUM, A.COD_DOCUM) COD_DOCUM, A99.NOM_TERCERO, A99.NOM2_TERCERO, A99.APE1_TERCERO, A99.APE2_TERCERO,
    ...
    JOIN ALIADOS.BOVEDA B ON B.TIP_DOCUM=COALESCE(a60.TIP_DOCUM, A.TIP_DOCUM) AND B.COD_DOCUM=COALESCE(a60.COD_DOCUM, A.COD_DOCUM) AND B.NUM_POLIZA=A.NUM_POLIZA AND B.NUM_SPTO<=A.NUM_SPTO AND B.STATUS>0
    JOIN A2990700 C 
...
@"SELECT C.NUM_RECIBO, C.FEC_EFEC_RECIBO, SUM(C.IMP_RECIBO) IMP_RECIBO, A400.COD_MON_ISO NOM_MON, A.NUM_POLIZA, A.COD_AGT, a1800.nom_ramo, a200.NOM_SECTOR, COALESCE(a60.TIP_DOCUM, A.TIP_DOCUM) TIP_DOCUM, COALESCE(a60.COD_DOCUM, A.COD_DOCUM) COD_DOCUM, A99.NOM_TERCERO, A99.NOM2_TERCERO, A99.APE1_TERCERO, A99.APE2_TERCERO,
    ...
    JOIN ALIADOS.BOVEDA B ON B.TIP_DOCUM=COALESCE(a60.TIP_DOCUM, A.TIP_DOCUM) AND B.COD_DOCUM=COALESCE(a60.COD_DOCUM, A.COD_DOCUM) AND B.NUM_POLIZA=A.NUM_POLIZA AND B.NUM_SPTO<=A.NUM_SPTO AND B.STATUS>0 AND B.STATUS<9 AND B.NextCollectAttempt<=TRUNC(:fec_efect_recibo2) 
    JOIN A2990700 C 
        ON C.NUM_RECIBO = (SELECT MIN(R.NUM_RECIBO)
                            FROM A2990700 R
                            WHERE R.COD_CIA = A.COD_CIA
    ...
    WHERE A.COD_CIA = :cod_cia " + filter +
        @" AND A.MCA_POLIZA_ANULADA  = 'N' AND A.MCA_PROVISIONAL = 'N'
        AND A.NUM_SPTO = ( SELECT MAX(A230.NUM_SPTO)
                            FROM A2000030 A230
                            WHERE A230.COD_CIA    = A.COD_CIA
                            AND A230.NUM_POLIZA = A.NUM_POLIZA )
