...
SELECT DISTINCT B.REASON,A99.MCA_FISICO, A99.TIP_DOCUM, A99.COD_DOCUM, A99.NOM_TERCERO, A99.NOM2_TERCERO, A99.APE1_TERCERO, A99.APE2_TERCERO, 
       ...
       TJ.TIP_TARJETA, A21.NOM_TIP_TARJETA, TJ.COD_TARJETA, A22.NOM_TARJETA, NTM2.NUM_TARJETA_ORIGINAL NUM_TARJETA, TJ.FEC_VCTO_TARJETA,
       TJ.NUM_POLIZA, TJ.NUM_SPTO
  FROM A1001399 A99
  JOIN A1001331 A31 ON A31.COD_CIA=A99.COD_CIA AND A31.TIP_DOCUM=A99.TIP_DOCUM AND A31.COD_DOCUM=A99.COD_DOCUM
  JOIN A1000802 TJ ON TJ.COD_CIA=A99.COD_CIA AND TJ.TIP_DOCUM=A99.TIP_DOCUM AND TJ.COD_DOCUM=A99.COD_DOCUM {filter} AND NOT TJ.NUM_TARJETA IS NULL AND NOT TJ.FEC_VCTO_TARJETA IS NULL 
        AND TJ.NUM_SPTO = (SELECT MAX(M.NUM_SPTO) FROM A1000802 M WHERE M.COD_CIA = TJ.COD_CIA AND M.NUM_POLIZA = TJ.NUM_POLIZA AND M.NUM_RIESGO = TJ.NUM_RIESGO )  
  JOIN NUM_TARJETA_MCR NTM2 ON NTM2.COD_CIA=A99.COD_CIA AND NTM2.TIP_DOCUM=A99.TIP_DOCUM AND NTM2.COD_DOCUM=A99.COD_DOCUM AND NTM2.NOM_TABLA='A1000802' AND NTM2.FEC_ACTU = (SELECT MAX(FEC_ACTU) FROM NUM_TARJETA_MCR NTM WHERE NTM.COD_CIA=A99.COD_CIA AND NTM.TIP_DOCUM=A99.TIP_DOCUM AND NTM.COD_DOCUM=A99.COD_DOCUM)
  ...
  LEFT JOIN ALIADOS.BOVEDA B ON B.TIP_DOCUM=A99.TIP_DOCUM AND B.COD_DOCUM=A99.COD_DOCUM AND B.NUM_POLIZA=TJ.NUM_POLIZA AND B.NUM_SPTO=TJ.NUM_SPTO
