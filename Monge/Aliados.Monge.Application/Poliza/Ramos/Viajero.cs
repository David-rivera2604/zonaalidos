    ...
    internal class Viajero
    {
        ...
        internal static async Task<Domain.Poliza.Emision.Respuesta> Handler(Domain.Poliza.Emision.Poliza risk, Architect.API.Core.Contracts.Security.Token tokenInfo)
        {
            ...
            try
            {
                ...
                List<Architect.API.Core.Contracts.General.Error> errors = Architect.API.Tron.Business.Cotizacion.Viajero.Validate(quote, tokenInfo, true);
                ...
                else
                {
                    ...
                    {
                        message_status = 400,
                        message_text = "Error(es) por validación. " + string.Join(", ", errors.Select(e => e.Message)),
                        message_id = Guid.NewGuid().ToString(),
                        document_id = risk.document_id
                    };
                }
