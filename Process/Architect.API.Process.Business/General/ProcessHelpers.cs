...
namespace Architect.API.Process.Business.General
{
    ...
    public static class ProcessHelpers
    {
        ...
        public static string FixData(bool apply)
        {
            ...
            List<Architect.API.Process.Contracts.General.ProcessCase> result = Architect.API.Process.DataAccess.General.ProcessCase.DataToFix();


            foreach (Architect.API.Process.Contracts.General.ProcessCase processCaseToFix in result)
            {
                ...
                if ((processCase.Status != 3 && processCase.Status != 6) && mainInstance.FinishDate.IsNotEmpty())
                {
                    ...
                    Contracts.General.ProcessSpecStep step = Architect.API.Process.Business.General.ProcessSpecStep.RetrieveById(100, lastInstance.StepId);

                    endInstance = instances.Where(i => i.StepId > 0 && i.TaskId == 0)
                        .OrderByDescending(i => i.ActivityId)
                        .FirstOrDefault();

                    Contracts.General.ProcessSpecStep endstep = Architect.API.Process.Business.General.ProcessSpecStep.RetrieveById(100, endInstance.StepId);
