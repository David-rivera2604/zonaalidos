...
namespace Architect.API.Process.Business.General
{
    ...
    public static class Process
    {
        ...
        public static Contracts.General.ProcessSpecFlow Specification(int flowId, int companyId, int customSLA)
        {
            ...
            {
                result = DataAccess.Specification.Retrieve(flowId, companyId, customSLA);
                if (result != null)
                {
        ...
        public static Core.Contracts.General.InstanceInformation CreateAutomaticInstance(Contracts.General.CreateProcessInstance newInstance, Core.Contracts.Security.Token token)
        {
            Core.Contracts.General.InstanceInformation result = null;
        ...
        public static Core.Contracts.General.InstanceInformation CurrentByEntity(int entityType, long entityId, int level, Core.Contracts.Security.Token tokenInfo)
        {
            return CurrentStep(InstanceRuntime(entityType, entityId, tokenInfo.CompanyId), level, tokenInfo);
        }

        public static Core.Contracts.General.InstanceInformation CurrentByInstance(int instanceId, int level, Core.Contracts.Security.Token tokenInfo)
        {
            return CurrentStep(InstanceRuntime(instanceId, tokenInfo.CompanyId), level, tokenInfo);
        }

        public static Contracts.General.ProcessInstance TaskCompletedByEntity(int entityType, long entityId, Core.Contracts.General.TaskChecked checkedInformation, int userId, int companyId)
        {
            return TaskCompleted(InstanceRuntime(entityType, entityId, companyId), checkedInformation, userId);
        }

        public static Contracts.General.ProcessInstance TaskCompletedByInstance(int instanceId, Core.Contracts.General.TaskChecked checkedInformation, int userId, int companyId)
        {
        ...
        private static void UpdateReferences(Core.Contracts.General.TaskChecked checkedInformation, int userId, int companyId, List<Contracts.General.ProcessInstance> runtime)
        {
        ...
        {
            return Architect.API.Process.DataAccess.General.ProcessInstance.CountByEntity(entityType, entityId, companyId) > 0;
        }

        public static Core.Contracts.General.InstanceInformation CurrentStep(List<Contracts.General.ProcessInstance> instance, int level, Core.Contracts.Security.Token tokenInfo)
        {
            Core.Contracts.General.InstanceInformation result = null;
            ...
            result = new Core.Contracts.General.InstanceInformation()
            {
                InstanceId = instance.First().InstanceId,
                CaseId = instance.First().CaseId,
                Tasks = new List<Core.Contracts.General.TaskInformation>()
            };

            result.Steps = new List<Core.Contracts.General.StepInformation>();
            List<Contracts.General.ProcessInstance> steps = null;
            Core.Contracts.General.StepInformation s = null;
            switch (level)
            {
                case 1:
            ...
            {
                s = new Core.Contracts.General.StepInformation()
                {
            ...
            foreach (Core.Contracts.General.StepInformation step in result.Steps.Where(i => i.ActivityId < lastActivityId && i.StartDate == null))
            {
        ...
        {
            List<Contracts.General.ProcessInstance> instance = Architect.API.Process.DataAccess.General.ProcessInstance.RetrieveByEntity(entityType, entityId, companyId);
        ...
        {
            List<Contracts.General.ProcessInstance> instance = Architect.API.Process.DataAccess.General.ProcessInstance.RetrieveByInstanceId(instanceId, companyId);
            if (instance == null || instance.Count == 0)
            {
                Architect.Utilities.Log.ErrorLog("InstanceRuntime", "no existe la instancia");
                throw new Exception("no existe la instancia");
        ...
        private static Contracts.General.ProcessInstance TaskCompleted(List<Contracts.General.ProcessInstance> instance, Core.Contracts.General.TaskChecked checkedInformation, int userId)
        {
            ...
            {
                foreach (Core.Contracts.General.Attachment attachment in checkedInformation.Attachments)
                {
        ...
        private static void Notify_ContactProcess_Progress(int caseId, Contracts.General.ProcessInstance step, Contracts.General.ProcessSpecFlow spec, int companyId, int userId, bool notify, string[] attachments)
        {
            ...
            if (mailFullList.Count > 0)
            {
                ...
                Core.Business.General.Mail.SendByTemplate(Core.Business.Common.LkpDescription(companyId, "MailServer", mailServer.ToString()), mailTemplate, companyId, userId, new { Case = procCase, Next = step, Spec = spec }, mailFullList, attachments);
            }
        }

        private static void Notify_ResponsibleProcess_Progress(int caseId, Contracts.General.ProcessInstance step, Contracts.General.ProcessSpecFlow spec, int companyId, int userId, string mailTemplateSetting = "Process.Core.Business.General.Mail.Responsible.Notify.Template")
        {
            Dictionary<string, string> mailFullList = new Dictionary<string, string>();
            Contracts.General.ProcessCase procCase = null;
            int mailServer = spec.MailServer;
        ...
        private static void Notify(int caseId, int responsible, string custom, int mailTmpl, int customMailServer, Contracts.General.ProcessSpecFlow spec, object entity, List<Core.Contracts.General.LookupValue> roles, int companyId, int userId, string mailTemplateSetting = "Process.Core.Business.General.Mail.Responsible.Notify.Template")
        {
            ...
            if (responsible == 1 || responsible == 3)
            {
                ...
                {
                    foreach (Core.Contracts.General.LookupValue stepRole in roles)
                    {
                        foreach (KeyValuePair<string, string> entry in Core.Business.Security.UserMember.EmailListByRolename(companyId, stepRole.Description))
                        {
                            if (!mailFullList.ContainsKey(entry.Key))
                            {
                                mailFullList.Add(entry.Key, entry.Value);
        ...
        public static void CaseComplement(int caseId, Core.Contracts.General.CaseComplement caseComplement, Core.Contracts.Security.Token token)
        {
        ...
        public static Core.Contracts.General.CaseComplement CaseComplement(int caseId, Core.Contracts.Security.Token token)
        {
            Core.Contracts.General.CaseComplement result = null;
            Contracts.General.ProcessCaseComplement info = ProcessCaseComplement.RetrieveById(caseId, token.CompanyId);
            if (info != null)
            {
                result = new Core.Contracts.General.CaseComplement()
                {
                    Fee = info.Fee,
        ...
        public static void SendNotification(int noteId, string note, bool contactNotify, bool responsibleNotify, int instanceId, string type, Core.Contracts.Security.Token tokenInfo)
        {
            try
            {
                // Obtener la instancia del proceso
                List<Contracts.General.ProcessInstance> instance = InstanceRuntime(instanceId, tokenInfo.CompanyId);
                if (instance == null || instance.Count == 0)
                {
                    Architect.Utilities.Log.ErrorLog("SendNotification", "No se encontró la instancia del proceso");
                    return;
                }

                Contracts.General.ProcessInstance currentFlow = instance.First(i => i.FlowId > 0 && i.StepId == 0 && i.TaskId == 0);
                Contracts.General.ProcessInstance currentStep = instance.FirstOrDefault(i => i.StepId > 0 && i.TaskId == 0 && i.StartDate > DateTime.MinValue && i.FinishDate == DateTime.MinValue);

                if (currentStep == null)
                {
                    currentStep = instance.FirstOrDefault(i => i.StepId > 0 && i.TaskId == 0);
                }

                Contracts.General.ProcessSpecFlow spec = Specification(currentFlow.FlowId, tokenInfo.CompanyId, currentFlow.SLA);
                Contracts.General.ProcessCase procCase = ProcessCase.RetrieveById(tokenInfo.CompanyId, currentFlow.CaseId);
                ...
                if (contactNotify && procCase?.ContactMainEmail.IsNotEmpty() == true)
                {
                    Dictionary<string, string> contactMailList = new Dictionary<string, string>
                    {
                        { procCase.ContactMainEmail, procCase.ContactMainName ?? string.Empty }
                    };

                    string contactMailTemplate = ConfigurationManager.AppSettings["Process.Core.Business.General.Mail.Contact.Note.Template"] ?? "Process.Core.Business.General.Mail.Contact.Notify.Template";

                    Core.Business.General.Mail.SendByTemplate(
                        Core.Business.Common.LkpDescription(tokenInfo.CompanyId, "MailServer", mailServer.ToString()),
                        contactMailTemplate,
                        tokenInfo.CompanyId,
                        tokenInfo.UserId,
                        mailData,
                ...
                if (responsibleNotify && currentStep?.Step != null)
                {
                    Dictionary<string, string> responsibleMailList = new Dictionary<string, string>();

                    // Obtener correos de los roles asignados al step
                    if (currentStep.Step.ProcessSpecStepRoles?.Count > 0)
                    {
                        foreach (Contracts.General.ProcessSpecStepRole stepRole in currentStep.Step.ProcessSpecStepRoles)
                        {
                            foreach (KeyValuePair<string, string> entry in Core.Business.Security.UserMember.EmailListByRolename(tokenInfo.CompanyId, stepRole.RoleName))
                            {
                                if (!responsibleMailList.ContainsKey(entry.Key))
                                {
                                    responsibleMailList.Add(entry.Key, entry.Value);
                                }
                            }
                        }
                    }

                    // Agregar correos personalizados si existen
                    if (currentStep.Step.MailToStepResponsibleCustom.IsNotEmpty())
                    {
                        foreach (string email in currentStep.Step.MailToStepResponsibleCustom.Split(','))
                        {
                            string cleanEmail = email.Trim();
                            if (cleanEmail.IsNotEmpty() && !responsibleMailList.ContainsKey(cleanEmail))
                            {
                                responsibleMailList.Add(cleanEmail, string.Empty);
                            }
                        }
                    }

                    if (responsibleMailList.Count > 0)
                    {
                        string responsibleMailTemplate = ConfigurationManager.AppSettings["Process.Core.Business.General.Mail.Responsible.Note.Template"] ?? "Process.Core.Business.General.Mail.Responsible.Notify.Template";

                        Core.Business.General.Mail.SendByTemplate(
                            Core.Business.Common.LkpDescription(tokenInfo.CompanyId, "MailServer", mailServer.ToString()),
                            responsibleMailTemplate,
                            tokenInfo.CompanyId,
                            tokenInfo.UserId,
                            mailData,
                            responsibleMailList
            ...
            {
                Architect.Utilities.Log.ErrorLog("SendNotification", "Error enviando notificaciones", ex);
                throw;
            }
        }
