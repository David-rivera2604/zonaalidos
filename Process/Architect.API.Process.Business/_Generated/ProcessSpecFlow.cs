...
namespace Architect.API.Process.Business.General
{
    ...
    public static partial class ProcessSpecFlow
    {
        ...
        public static Architect.API.Process.Contracts.General.ProcessSpecFlowResult CreateOrUpdate(int companyId, int userId, Architect.API.Process.Contracts.General.ProcessSpecFlow item)
        {
            if (item.Id.IsEmpty() || Architect.API.Process.DataAccess.General.ProcessSpecFlow.Count(item.Id, companyId) == 0)
            {
        ...
        public static Architect.API.Process.Contracts.General.ProcessSpecFlowResult Create(int companyId, int userId, Architect.API.Process.Contracts.General.ProcessSpecFlow item)
        {
            Architect.API.Process.Contracts.General.ProcessSpecFlow result = item;
            List<Core.Contracts.General.Error> errors = Architect.API.Process.Business.General.ProcessSpecFlow.Validate(companyId, item, true, false);

            if (errors.Count == 0)
            {
                if (result.Id.IsEmpty())
                {
                    result.Id = Architect.API.Process.DataAccess.General.ProcessSpecFlow.RetrieveLastKey() + 1;
                }
                result.CompanyId = companyId;
                result.UpdateUserCode = userId;
                result.UpdateDate = DateTime.Now;

                if (Architect.API.Process.DataAccess.General.ProcessSpecFlow.Create(result) > 0)
                {
            ...
            return new Architect.API.Process.Contracts.General.ProcessSpecFlowResult() { ProcessSpecFlow = result, Errors = errors };
        }
        ...
        public static List<Architect.API.Process.Contracts.General.ProcessSpecFlow> Retrieve(int companyId, string filter, int beginIndex, int endIndex)
        {
            List<Architect.API.Process.Contracts.General.ProcessSpecFlow> result = Architect.API.Process.DataAccess.General.ProcessSpecFlow.RetrieveAll(companyId, Architect.API.Process.DataAccess.General.ProcessSpecFlow.FilterBuilderEx(filter, false), beginIndex, endIndex);

            foreach (Architect.API.Process.Contracts.General.ProcessSpecFlow item in result)
            {
        ...
        public static Architect.API.Process.Contracts.General.ProcessSpecFlow RetrieveById(int companyId, int id)
        {
            IDbConnection currentConnection = Architect.DataFactory.Database.OpenConnection("Research");
            Architect.API.Process.Contracts.General.ProcessSpecFlow result = Architect.API.Process.DataAccess.General.ProcessSpecFlow.Retrieve(id, companyId, currentConnection);

            if (result.IsNotEmpty())
            {
                List<Architect.API.Process.Contracts.General.ProcessSpecFlowRole> internalRoles = DataAccess.General.ProcessSpecFlowRole.RetrieveByStepId(id, currentConnection);
        ...
        public static string ConditionResolve(string condition, Architect.API.Process.Contracts.General.ProcessSpecFlow processSpecFlow)
        {
        ...
        public static Architect.API.Process.Contracts.General.ProcessSpecFlowResult Update(int companyId, int userId, int id, Architect.API.Process.Contracts.General.ProcessSpecFlow item)
        {
            Architect.API.Process.Contracts.General.ProcessSpecFlow result = null;
            List<Core.Contracts.General.Error> errors = Architect.API.Process.Business.General.ProcessSpecFlow.Validate(companyId, item, false, false);
            List<Core.Contracts.General.Error> warnings = null;
            ...
            if (errors.Count == 0)
            {
                ...
                if (Architect.API.Process.DataAccess.General.ProcessSpecFlow.Update(result) > 0)
                {
                    SynchronizeRoles(companyId, userId, result.Id, result.Roles);
            ...
            return new Architect.API.Process.Contracts.General.ProcessSpecFlowResult() { ProcessSpecFlow = result, Errors = errors, Warnings = warnings };
        }
        ...
        public static Architect.API.Process.Contracts.General.ProcessSpecFlowResult Delete(int companyId, int userId, int id)
        {
            Architect.API.Process.Contracts.General.ProcessSpecFlow result = null;
            List<Core.Contracts.General.Error> errors = Architect.API.Process.Business.General.ProcessSpecFlow.Validate(companyId, new Architect.API.Process.Contracts.General.ProcessSpecFlow() { Id = id }, false, true);

            if (errors.Count == 0)
            {
                result = Architect.API.Process.DataAccess.General.ProcessSpecFlow.Retrieve(id, companyId);
                if (result.IsNotEmpty() && Architect.API.Process.DataAccess.General.ProcessSpecFlow.Delete(id, companyId) > 0)
                {
        ...
        {
            return Architect.API.Process.DataAccess.General.ProcessSpecFlow.Count(Architect.API.Process.DataAccess.General.ProcessSpecFlow.FilterBuilderEx(filter));
        }
        ...
        public static List<Core.Contracts.General.Error> Validate(int companyId, Architect.API.Process.Contracts.General.ProcessSpecFlow source, bool isnew, bool isdelete)
        {
            ...
            if ((!isnew || isdelete) && source.Id.IsNotEmpty() && Architect.API.Process.DataAccess.General.ProcessSpecFlow.Count(source.Id, companyId) == 0)
            {
                result.Add(new Core.Contracts.General.Error() { Group = group, Key = "Id", Message = "La proceso no está registrada" });
            }
            if (isnew && source.Id.IsNotEmpty() && Architect.API.Process.DataAccess.General.ProcessSpecFlow.Count(source.Id, companyId) > 0)
            {
        ...
        /// <param name="companyId">Identificación de la compañía propietaria.</param>
        /// <param name="item">Instancia de ProcessSpecFlow</param>
        private static void MapLookups(int companyId, Architect.API.Process.Contracts.General.ProcessSpecFlow item)
        {
            if (item.IsEmpty())
            {
                return;
        ...
        public static List<Architect.Utilities.Contracts.LookUpValue> RetrieveReferencesById(int companyId, int id)
        {
            ...
            IDbConnection currentConnection = Architect.DataFactory.Database.OpenConnection("Research");
            Architect.API.Process.Contracts.General.ProcessSpecFlow process = Architect.API.Process.DataAccess.General.ProcessSpecFlow.Retrieve(id, companyId, currentConnection);
