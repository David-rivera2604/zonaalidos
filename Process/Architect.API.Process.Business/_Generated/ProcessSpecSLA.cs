...
namespace Architect.API.Process.Business.General
{
    ...
    public static partial class ProcessSpecSLA
    {
        ...
        public static Architect.API.Process.Contracts.General.ProcessSpecSLAResult CreateOrUpdate(int companyId, int userId, Architect.API.Process.Contracts.General.ProcessSpecSLA item)
        {
            if (item.Id.IsEmpty() || Architect.API.Process.DataAccess.General.ProcessSpecSLA.Count(item.Id, companyId) == 0)
            {
        ...
        public static Architect.API.Process.Contracts.General.ProcessSpecSLAResult Create(int companyId, int userId, Architect.API.Process.Contracts.General.ProcessSpecSLA item)
        {
            Architect.API.Process.Contracts.General.ProcessSpecSLA result = item;
            List<Core.Contracts.General.Error> errors = Architect.API.Process.Business.General.ProcessSpecSLA.Validate(companyId, item, true, false);

            if (errors.Count == 0)
            {
                if (result.Id.IsEmpty())
                {
                    result.Id = Architect.API.Process.DataAccess.General.ProcessSpecSLA.RetrieveLastKey() + 1;
                }
                result.CompanyId = companyId;
                result.UpdateUserCode = userId;
                result.UpdateDate = DateTime.Now;

                if (Architect.API.Process.DataAccess.General.ProcessSpecSLA.Create(result) > 0)
                {
            ...
            return new Architect.API.Process.Contracts.General.ProcessSpecSLAResult() { ProcessSpecSLA = result, Errors = errors };
        }
        ...
        public static List<Architect.API.Process.Contracts.General.ProcessSpecSLA> Retrieve(int companyId, string filter, int beginIndex, int endIndex)
        {
            List<Architect.API.Process.Contracts.General.ProcessSpecSLA> result = Architect.API.Process.DataAccess.General.ProcessSpecSLA.RetrieveAll(companyId, Architect.API.Process.DataAccess.General.ProcessSpecSLA.FilterBuilder(filter, false), beginIndex, endIndex);

            foreach (Architect.API.Process.Contracts.General.ProcessSpecSLA item in result)
            {
        ...
        public static Architect.API.Process.Contracts.General.ProcessSpecSLA RetrieveById(int companyId, int id)
        {
            Architect.API.Process.Contracts.General.ProcessSpecSLA result = Architect.API.Process.DataAccess.General.ProcessSpecSLA.Retrieve(id, companyId);
        ...
        public static Architect.API.Process.Contracts.General.ProcessSpecSLAResult Update(int companyId, int userId, int id, Architect.API.Process.Contracts.General.ProcessSpecSLA item)
        {
            Architect.API.Process.Contracts.General.ProcessSpecSLA result = null;
            List<Core.Contracts.General.Error> errors = Architect.API.Process.Business.General.ProcessSpecSLA.Validate(companyId, item, false, false);
            ...
            if (errors.Count == 0)
            {
                ...
                if (Architect.API.Process.DataAccess.General.ProcessSpecSLA.Update(result) > 0)
                {
                    MapLookups(companyId, result);
        ...
        public static Architect.API.Process.Contracts.General.ProcessSpecSLAResult Delete(int companyId, int userId, int id)
        {
            Architect.API.Process.Contracts.General.ProcessSpecSLA result = null;
            List<Core.Contracts.General.Error> errors = Architect.API.Process.Business.General.ProcessSpecSLA.Validate(companyId, new Architect.API.Process.Contracts.General.ProcessSpecSLA() { Id = id }, false, true);
            ...
            if (errors.Count == 0)
            {
                ...
                result = Architect.API.Process.DataAccess.General.ProcessSpecSLA.Retrieve(id, companyId);
                if (result.IsNotEmpty() && Architect.API.Process.DataAccess.General.ProcessSpecSLA.Delete(id, companyId) > 0)
                {
        ...
        {
            return Architect.API.Process.DataAccess.General.ProcessSpecSLA.Count(Architect.API.Process.DataAccess.General.ProcessSpecSLA.FilterBuilder(filter));
        }
        ...
        public static List<Core.Contracts.General.Error> Validate(int companyId, Architect.API.Process.Contracts.General.ProcessSpecSLA source, bool isnew, bool isdelete)
        {
            ...
            if ((!isnew || isdelete) && source.Id.IsNotEmpty() && Architect.API.Process.DataAccess.General.ProcessSpecSLA.Count(source.Id, companyId) == 0)
            {
                result.Add(new Core.Contracts.General.Error() { Group = group, Key = "Id", Message = "La proceso no está registrada" });
            }
            if (isnew && source.Id.IsNotEmpty() && Architect.API.Process.DataAccess.General.ProcessSpecSLA.Count(source.Id, companyId) > 0)
            {
        ...
        /// <param name="companyId">Identificación de la compañía propietaria.</param>
        /// <param name="item">Instancia de ProcessSpecSLA</param>
        private static void MapLookups(int companyId, Architect.API.Process.Contracts.General.ProcessSpecSLA item)
        {
            if (item.IsEmpty())
            {
                return;
        ...
        /// <param name="isdelete">Indicador que se quiere eliminar el registro.</param>
        /// <returns>Lista de errores/notificaciones resultantes de las validaciones.</returns>
        public static List<Core.Contracts.General.Error> Validate22(int companyId, Architect.API.Process.Contracts.General.ProcessSpecSLALevel source, bool isnew, bool isdelete)
        {
        ...
        private static void MapLookups2(int companyId, Architect.API.Process.Contracts.General.ProcessSpecSLALevel item)
        {
            if (item.IsEmpty())
            {
                return;
