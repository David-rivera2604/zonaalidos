...
namespace Architect.API.Process.Business.General
{
    ...
    public static partial class ProcessSpecStep
    {
        ...
        public static Architect.API.Process.Contracts.General.ProcessSpecStepResult CreateOrUpdate(int companyId, int userId, Architect.API.Process.Contracts.General.ProcessSpecStep item)
        {
            if (item.Id.IsEmpty() || Architect.API.Process.DataAccess.General.ProcessSpecStep.Count(item.Id, companyId) == 0)
            {
        ...
        public static Architect.API.Process.Contracts.General.ProcessSpecStepResult Create(int companyId, int userId, Architect.API.Process.Contracts.General.ProcessSpecStep item)
        {
            Architect.API.Process.Contracts.General.ProcessSpecStep result = item;
            List<Core.Contracts.General.Error> errors = Architect.API.Process.Business.General.ProcessSpecStep.Validate(companyId, item, true, false);

            if (errors.Count == 0)
            {
                if (result.Id.IsEmpty())
                {
                    result.Id = Architect.API.Process.DataAccess.General.ProcessSpecStep.RetrieveLastKey() + 1;
                }
                ...
                if (Architect.API.Process.DataAccess.General.ProcessSpecStep.Create(result) > 0)
                {
            ...
            return new Architect.API.Process.Contracts.General.ProcessSpecStepResult() { ProcessSpecStep = result, Errors = errors };
        }
        ...
        public static List<Architect.API.Process.Contracts.General.ProcessSpecStep> Retrieve(int companyId, string filter, int beginIndex, int endIndex)
        {
            List<Architect.API.Process.Contracts.General.ProcessSpecStep> result = Architect.API.Process.DataAccess.General.ProcessSpecStep.RetrieveAll(companyId, Architect.API.Process.DataAccess.General.ProcessSpecStep.FilterBuilderEx(filter), beginIndex, endIndex);
        ...
        public static Architect.API.Process.Contracts.General.ProcessSpecStep RetrieveById(int companyId, int id)
        {
            IDbConnection currentConnection = Architect.DataFactory.Database.OpenConnection("Research");

            Architect.API.Process.Contracts.General.ProcessSpecStep result = Architect.API.Process.DataAccess.General.ProcessSpecStep.RetrieveFull(id, companyId, currentConnection);

            if (result.IsNotEmpty())
            {
                result.ProcessSpecStepRoles = DataAccess.General.ProcessSpecStepRole.RetrieveByStepId(id, currentConnection);
                if (result.ProcessSpecStepRoles.Count > 0)
                {
                    result.Roles = new List<Architect.Utilities.Contracts.LookUpValue>();
                    foreach (Architect.API.Process.Contracts.General.ProcessSpecStepRole item in result.ProcessSpecStepRoles)
                    {
        ...
        public static Architect.API.Process.Contracts.General.ProcessSpecStepResult Update(int companyId, int userId, int id, Architect.API.Process.Contracts.General.ProcessSpecStep item)
        {
            Architect.API.Process.Contracts.General.ProcessSpecStep result = null;
            List<Core.Contracts.General.Error> errors = Architect.API.Process.Business.General.ProcessSpecStep.Validate(companyId, item, false, false);
            ...
            if (errors.Count == 0)
            {
                ...
                if (Architect.API.Process.DataAccess.General.ProcessSpecStep.Update(result) > 0)
                {
                    SynchronizeRoles(companyId, userId, result.Id, result.Roles);
                    MapLookups(companyId, result);
        ...
        public static Architect.API.Process.Contracts.General.ProcessSpecStepResult Delete(int companyId, int userId, int id)
        {
            Architect.API.Process.Contracts.General.ProcessSpecStep result = null;
            List<Core.Contracts.General.Error> errors = Architect.API.Process.Business.General.ProcessSpecStep.Validate(companyId, new Architect.API.Process.Contracts.General.ProcessSpecStep() { Id = id }, false, true);

            if (errors.Count == 0)
            {
                result = Architect.API.Process.DataAccess.General.ProcessSpecStep.Retrieve(id, companyId);

                if (result.IsNotEmpty() && Architect.API.Process.DataAccess.General.ProcessSpecStep.Delete(id, companyId) > 0)
                {
        ...
        {
            return Architect.API.Process.DataAccess.General.ProcessSpecStep.Count(Architect.API.Process.DataAccess.General.ProcessSpecStep.FilterBuilderEx(filter));
        }
        ...
        public static List<Core.Contracts.General.Error> Validate(int companyId, Architect.API.Process.Contracts.General.ProcessSpecStep source, bool isnew, bool isdelete)
        {
            ...
            if ((!isnew || isdelete) && source.Id.IsNotEmpty() && Architect.API.Process.DataAccess.General.ProcessSpecStep.Count(source.Id, companyId) == 0)
            {
                result.Add(new Core.Contracts.General.Error() { Group = group, Key = "Id", Message = "La paso no está registrada" });
            }
            if (isnew && source.Id.IsNotEmpty() && Architect.API.Process.DataAccess.General.ProcessSpecStep.Count(source.Id, companyId) > 0)
            {
        ...
        /// <param name="companyId">Identificación de la compañía propietaria.</param>
        /// <param name="item">Instancia de ProcessSpecStep</param>
        private static void MapLookups(int companyId, Architect.API.Process.Contracts.General.ProcessSpecStep item)
        {
            if (item.IsEmpty())
            {
                return;
        ...
        {
            List<Architect.API.Process.Contracts.General.ProcessSpecStepRole> roles = DataAccess.General.ProcessSpecStepRole.RetrieveByStepId(id);
            if (roles.IsEmpty())
            {
                roles = new List<Architect.API.Process.Contracts.General.ProcessSpecStepRole>();
            }
            if (currentRoles.IsNotEmpty())
            {
                //Agrega un nuevo registro o se cambia uno existente                
                Architect.API.Process.Contracts.General.ProcessSpecStepRole toAdd = null;
                foreach (Architect.Utilities.Contracts.LookUpValue newItem in currentRoles)
                {
                    ...
                    if (toAdd.IsEmpty())
                    {
                        toAdd = new Architect.API.Process.Contracts.General.ProcessSpecStepRole();
                        roles.Add(toAdd);
                        toAdd.Id = id;
                        ...
                        DataAccess.General.ProcessSpecStepRole.Create(toAdd);
                    }
                }
            }
            ...
            if (roles?.Count > 0)
            {
                foreach (Architect.API.Process.Contracts.General.ProcessSpecStepRole currentRole in roles)
                {
                    if (currentRoles.Find(r => r.Code == currentRole.RoleId.ToString()).IsEmpty())
                    {
                        DataAccess.General.ProcessSpecStepRole.DeleteWithRole(id, currentRole.RoleId, companyId);
                    }
                }
            }
        }
