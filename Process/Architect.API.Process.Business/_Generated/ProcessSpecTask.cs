...
namespace Architect.API.Process.Business.General
{
    ...
    public static partial class ProcessSpecTask
    {
        ...
        public static Architect.API.Process.Contracts.General.ProcessSpecTaskResult CreateOrUpdate(int companyId, int userId, Architect.API.Process.Contracts.General.ProcessSpecTask item)
        {
            if (item.Id.IsEmpty() || Architect.API.Process.DataAccess.General.ProcessSpecTask.Count(item.Id, companyId) == 0)
            {
        ...
        public static Architect.API.Process.Contracts.General.ProcessSpecTaskResult Create(int companyId, int userId, Architect.API.Process.Contracts.General.ProcessSpecTask item)
        {
            Architect.API.Process.Contracts.General.ProcessSpecTask result = item;
            List<Core.Contracts.General.Error> errors = Architect.API.Process.Business.General.ProcessSpecTask.Validate(companyId, item, true, false);

            if (errors.Count == 0)
            {
                if (result.Id.IsEmpty())
                {
                    result.Id = Architect.API.Process.DataAccess.General.ProcessSpecTask.RetrieveLastKey() + 1;
                }
                ...
                if (Architect.API.Process.DataAccess.General.ProcessSpecTask.Create(result) > 0)
                {
            ...
            return new Architect.API.Process.Contracts.General.ProcessSpecTaskResult() { ProcessSpecTask = result, Errors = errors };
        }
        ...
        public static List<Architect.API.Process.Contracts.General.ProcessSpecTask> Retrieve(int companyId, string filter, int beginIndex, int endIndex)
        {
            List<Architect.API.Process.Contracts.General.ProcessSpecTask> result = Architect.API.Process.DataAccess.General.ProcessSpecTask.RetrieveAll(companyId, Architect.API.Process.DataAccess.General.ProcessSpecTask.FilterBuilderEx(filter), beginIndex, endIndex);

            foreach (Architect.API.Process.Contracts.General.ProcessSpecTask item in result)
            {
        ...
        public static Architect.API.Process.Contracts.General.ProcessSpecTask RetrieveById(int companyId, int id)
        {
            Architect.API.Process.Contracts.General.ProcessSpecTask result = Architect.API.Process.DataAccess.General.ProcessSpecTask.RetrieveFull(id, companyId);
        ...
        public static Architect.API.Process.Contracts.General.ProcessSpecTaskResult Update(int companyId, int userId, int id, Architect.API.Process.Contracts.General.ProcessSpecTask item)
        {
            Architect.API.Process.Contracts.General.ProcessSpecTask result = null;
            List<Core.Contracts.General.Error> errors = Architect.API.Process.Business.General.ProcessSpecTask.Validate(companyId, item, false, false);
            ...
            if (errors.Count == 0)
            {
                ...
                if (Architect.API.Process.DataAccess.General.ProcessSpecTask.Update(result) > 0)
                {
                    MapLookups(companyId, result);
        ...
        public static Architect.API.Process.Contracts.General.ProcessSpecTaskResult Delete(int companyId, int userId, int id)
        {
            Architect.API.Process.Contracts.General.ProcessSpecTask result = null;
            List<Core.Contracts.General.Error> errors = Architect.API.Process.Business.General.ProcessSpecTask.Validate(companyId, new Architect.API.Process.Contracts.General.ProcessSpecTask() { Id = id }, false, true);

            if (errors.Count == 0)
            {
                result = Architect.API.Process.DataAccess.General.ProcessSpecTask.Retrieve(id, companyId);
                if (result.IsNotEmpty() && Architect.API.Process.DataAccess.General.ProcessSpecTask.Delete(id, companyId) > 0)
                {
        ...
        {
            return Architect.API.Process.DataAccess.General.ProcessSpecTask.Count(Architect.API.Process.DataAccess.General.ProcessSpecTask.FilterBuilderEx(filter));
        }
        ...
        public static List<Core.Contracts.General.Error> Validate(int companyId, Architect.API.Process.Contracts.General.ProcessSpecTask source, bool isnew, bool isdelete)
        {
            ...
            if ((!isnew || isdelete) && source.Id.IsNotEmpty() && Architect.API.Process.DataAccess.General.ProcessSpecTask.Count(source.Id, companyId) == 0)
            {
                result.Add(new Core.Contracts.General.Error() { Group = group, Key = "Id", Message = "La tarea no está registrada" });
            }
            if (isnew && source.Id.IsNotEmpty() && Architect.API.Process.DataAccess.General.ProcessSpecTask.Count(source.Id, companyId) > 0)
            {
        ...
        /// <param name="companyId">Identificación de la compañía propietaria.</param>
        /// <param name="item">Instancia de ProcessSpecTask</param>
        private static void MapLookups(int companyId, Architect.API.Process.Contracts.General.ProcessSpecTask item)
        {
            if (item.IsEmpty())
            {
                return;
