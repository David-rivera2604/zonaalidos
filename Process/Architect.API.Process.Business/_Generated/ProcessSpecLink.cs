...
namespace Architect.API.Process.Business.General
{
    ...
    public static partial class ProcessSpecLink
    {
        ...
        public static Architect.API.Process.Contracts.General.ProcessSpecLinkResult CreateOrUpdate(int companyId, int userId, Architect.API.Process.Contracts.General.ProcessSpecLink item)
        {
            if (item.Id.IsEmpty() || Architect.API.Process.DataAccess.General.ProcessSpecLink.Count(item.Id, companyId) == 0)
            {
        ...
        public static Architect.API.Process.Contracts.General.ProcessSpecLinkResult Create(int companyId, int userId, Architect.API.Process.Contracts.General.ProcessSpecLink item)
        {
            Architect.API.Process.Contracts.General.ProcessSpecLink result = item;
            List<Core.Contracts.General.Error> errors = Architect.API.Process.Business.General.ProcessSpecLink.Validate(companyId, item, true, false);

            if (errors.Count == 0)
            {
                if (result.Id.IsEmpty())
                {
                    result.Id = Architect.API.Process.DataAccess.General.ProcessSpecLink.RetrieveLastKey() + 1;
                }
                ...
                if (Architect.API.Process.DataAccess.General.ProcessSpecLink.Create(result) > 0)
                {
            ...
            return new Architect.API.Process.Contracts.General.ProcessSpecLinkResult() { ProcessSpecLink = result, Errors = errors };
        }
        ...
        public static List<Architect.API.Process.Contracts.General.ProcessSpecLink> Retrieve(int companyId, string filter, int beginIndex, int endIndex)
        {
            List<Architect.API.Process.Contracts.General.ProcessSpecLink> result = Architect.API.Process.DataAccess.General.ProcessSpecLink.RetrieveAll(companyId, DataAccess.General.ProcessSpecLink.FilterBuilderFull(filter, false), beginIndex, endIndex);
            result = result.OrderBy(c => c.Type).ThenBy(person => person.LinkOrder).ToList();
            foreach (Architect.API.Process.Contracts.General.ProcessSpecLink item in result)
            {
        ...
        public static Architect.API.Process.Contracts.General.ProcessSpecLink RetrieveById(int companyId, int id)
        {
            Architect.API.Process.Contracts.General.ProcessSpecLink result = Architect.API.Process.DataAccess.General.ProcessSpecLink.Retrieve(id, companyId);
        ...
        public static Architect.API.Process.Contracts.General.ProcessSpecLinkResult Update(int companyId, int userId, int id, Architect.API.Process.Contracts.General.ProcessSpecLink item)
        {
            Architect.API.Process.Contracts.General.ProcessSpecLink result = null;
            List<Core.Contracts.General.Error> errors = Architect.API.Process.Business.General.ProcessSpecLink.Validate(companyId, item, false, false);
            ...
            if (errors.Count == 0)
            {
                ...
                if (Architect.API.Process.DataAccess.General.ProcessSpecLink.Update(result) > 0)
                {
                    MapLookups(companyId, result);
        ...
        public static Architect.API.Process.Contracts.General.ProcessSpecLinkResult Delete(int companyId, int userId, int id)
        {
            Architect.API.Process.Contracts.General.ProcessSpecLink result = null;
            List<Core.Contracts.General.Error> errors = Architect.API.Process.Business.General.ProcessSpecLink.Validate(companyId, new Architect.API.Process.Contracts.General.ProcessSpecLink() { Id = id }, false, true);

            if (errors.Count == 0)
            {
                result = Architect.API.Process.DataAccess.General.ProcessSpecLink.Retrieve(id, companyId);
                if (result.IsNotEmpty() && Architect.API.Process.DataAccess.General.ProcessSpecLink.Delete(id, companyId) > 0)
                {
        ...
        {
            return Architect.API.Process.DataAccess.General.ProcessSpecLink.Count(Architect.API.Process.DataAccess.General.ProcessSpecLink.FilterBuilder(filter));
        }
        ...
        public static List<Core.Contracts.General.Error> Validate(int companyId, Architect.API.Process.Contracts.General.ProcessSpecLink source, bool isnew, bool isdelete)
        {
            ...
            if ((!isnew || isdelete) && source.Id.IsNotEmpty() && Architect.API.Process.DataAccess.General.ProcessSpecLink.Count(source.Id, companyId) == 0)
            {
                result.Add(new Core.Contracts.General.Error() { Group = group, Key = "Id", Message = "La id no está registrada" });
            }
            if (isnew && source.Id.IsNotEmpty() && Architect.API.Process.DataAccess.General.ProcessSpecLink.Count(source.Id, companyId) > 0)
            {
        ...
        /// <param name="companyId">Identificación de la compañía propietaria.</param>
        /// <param name="item">Instancia de ProcessSpecLink</param>
        private static void MapLookups(int companyId, Architect.API.Process.Contracts.General.ProcessSpecLink item)
        {
            if (item.IsEmpty())
            {
                return;
